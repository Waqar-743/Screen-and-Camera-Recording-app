...
impl VideoEncoder {
    ...
    pub fn new<P: AsRef<Path>>(
        ...
        // The stream descriptor describes the *input* sample format we provide via `send_audio_buffer`.
        // We provide PCM samples and let the `MediaTranscoder` encode per `media_encoding_profile`.
        let audio_input_properties = AudioEncodingProperties::CreatePcm(
            audio_encoding_properties.SampleRate()?,
            audio_encoding_properties.ChannelCount()?,
            audio_encoding_properties.BitsPerSample()?,
        )?;
        let audio_stream_descriptor = AudioStreamDescriptor::Create(&audio_input_properties)?;
    ...
    /// Sends a video frame buffer and a PCM audio buffer for encoding.
    ///
    /// This queues both video and audio samples before waiting, so it does not depend on the
    /// internal sample request order (audio-first vs video-first).
    #[inline]
    pub fn send_frame_buffer_with_audio(
        &mut self,
        video_buffer: &[u8],
        audio_buffer: &[u8],
        timestamp: i64,
        ...
        let ts = match self.first_timestamp {
            Some(timestamp) => TimeSpan { Duration: base_timestamp - timestamp.Duration },
            None => {
                self.first_timestamp = Some(TimeSpan { Duration: base_timestamp });
                TimeSpan { Duration: 0 }
            }
        };

        self.frame_sender.send(Some((
            VideoEncoderSource::Buffer((SendDirectX::new(video_buffer.as_ptr()), video_buffer.len())),
            ts,
        )))?;
        self.audio_sender.send(Some((
        ...
        // Wait for both samples to be consumed.
        let (lock, cvar) = &*self.frame_notify;
        let mut processed = lock.lock();
        if !*processed {
            cvar.wait(&mut processed);
